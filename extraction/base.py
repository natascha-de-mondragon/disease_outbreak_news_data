...
def run(
    ...
    *,
    ...
    Clears the table first, so a clean slate gives idempotency without a unique
    constraint on id_outbreak.
    """
    ...
    for identifier, publication, title, content in dons:
        try:
    ...
    return {"dons_processed": len(dons), "outbreak_rows": len(rows), **counters}
